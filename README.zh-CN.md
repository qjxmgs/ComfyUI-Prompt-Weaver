...
后端还支持第三层 SQLite 缺失翻译补充源：按用户指定，它通过 GitHub Contents API 固定跟踪上游 [`ffdkj/ffdkj-Danbooru_Tag-Chinese-English-Translation-Table`](https://github.com/ffdkj/ffdkj-Danbooru_Tag-Chinese-English-Translation-Table)，仅在用户手动更新时把 `tag.sqlite` 流式下载到当前 ComfyUI 用户目录。插件内置的补充源策略会覆盖 Prompt Weaver 远程清单中的旧配置；从旧镜像切换时强制重新下载，此后 blob 未变化才跳过。数据库需通过大小、SQLite 完整性、schema、行数和字段约束校验才会原子替换，失败继续使用上一个本地可用版本。加载时只按主键查询基础词库中主翻译仍为空的标签，绝不覆盖主翻译，也不引入库外标签、分类或热度。上游仓库目前仍未声明数据许可证，因此清单记录为 `license_status: user-directed`，不会将其标为 MIT 或声称拥有再分发权；管理面板会明确显示“已安装供本地使用”。`/prompt-weaver/tag-autocomplete/status` 是纯本地只读接口，并报告实际基础词库交集的主翻译数、合并翻译数、覆盖率，以及补充层的许可状态、可用状态、补齐数量、blob SHA、更新时间和独立错误信息。
