...
编辑器标题栏左侧显示当前已选数量、可编辑的卡片标题输入框，并保留“文本模式”和“保留未选”开关。标题草稿只在点击“确认”后随提示词一起保存；从编辑器打开收藏操作时会使用当前标题草稿。悬停两个开关时会根据当前勾选状态提示下一步操作或未选提示词的保存结果。编辑器内无论焦点位于标题、提示词、按钮还是联想候选，普通 Tab 和 Shift+Tab 都固定用于切换文本模式与标签模式，不再移动焦点或确认联想候选。两个开关后方增加撤销、重做图标按钮，只在本次编辑窗口内保存最多 100 步提示词内容历史，确认、取消或关闭窗口后立即清空。标签切换、添加、删除、联想插入、全开/全关及已结算的文本编辑会进入历史；标题、模式、“保留未选”、字号和窗口几何不会进入历史。连续输入会在失焦或提交时合并为一步。可使用 Ctrl/Cmd+Z 撤销、Ctrl/Cmd+Shift+Z 重做，Windows/Linux 还支持 Ctrl+Y；输入框仍处于未失焦编辑状态时保留浏览器原生文本撤销。右侧依次显示“字号”文字、12–30px 调整滑杆、实时像素值，以及与联想浮层一致的红色关闭按钮。字号会保存在本地。编辑器最小宽度为 600px，可拖动标题栏非交互区域移动，也可从右下角调整尺寸；再次打开时会恢复保存的窗口位置与尺寸，并自动限制在当前视口范围内。
