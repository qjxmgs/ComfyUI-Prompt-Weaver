...
编辑器底栏的“收藏”会打开“一级分类 → 二级分类 → 我的收藏”三栏管理面板，用于保存和管理当前正在编辑的卡片。鼠标悬停或点击二级分类都会立即显示其中的收藏；“我的收藏”标题右侧的 `+` 会把当前草稿另存为该分类中的独立收藏，即使草稿已经关联其他收藏也允许再次保存微调版本。每张收藏都会显示实际输出提示词数量，并提供与网格级联菜单一致的双语 Tooltip 和红色二次确认删除按钮。收藏内容区只负责展示；只有点击右侧圆角“覆盖”按钮才会弹窗确认，并使用当前编辑草稿覆盖该收藏快照和建立关联。收藏可拖到二级分类上移动且不修改快照；悬停一级分类只展开其二级列表，键盘、触屏和窄窗口仍可使用右键菜单中的“移动”。空 Prompt 不能新建或覆盖收藏；请求失败时会留在面板中提示，但不会阻止编辑、复制或确认。

收藏库中的创建、移动、更新和移除操作会立即生效；当前网格卡片的内容与收藏关联只有点击“确认”后才提交。取消编辑会保持卡片打开前的配置，但不会回滚已经完成的全局收藏库操作。收藏始终是独立快照，后续修改卡片不会隐式覆盖收藏内容。

收藏库写入当前 ComfyUI 用户目录下的 `ComfyUI-Prompt-Weaver/prompt-card-library.json`，沿用存档服务的锁、严格校验、临时文件和原子替换策略；文件损坏或读取失败时绝不覆盖原文件。上限为 100 个一级分类、500 个二级分类、2,000 张收藏卡片和 20 MiB 文件。v1 按创建顺序展示，暂不提供分类/收藏拖拽排序、搜索、导入或导出。
