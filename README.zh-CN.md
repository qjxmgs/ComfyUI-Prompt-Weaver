...
提示词卡片收藏是按 ComfyUI 用户隔离的全局资料库，同一用户的所有提示词网格节点、Workflow、存档和浏览器标签页共同使用。每张收藏必须挂在某个一级分类下的二级分类中；一级、二级分类都可以随时新建和改名。空分类可直接删除；删除包含收藏的分类分支前必须选择另一个二级分类，后端会在一次原子操作中迁移收藏并删除整个分支。同级分类名称不区分大小写且不可重复。
...
测试覆盖节点配置解析、插件注册与路由、全局存档、两级提示词卡片收藏库、网格排序、收藏追加与去重、提示词编辑器、双源提示词联想、词库校验与失败回退、语言资源、语言切换和历史数据兼容。
