...
编辑器标题栏左侧显示当前已选数量，并保留“文本模式”和“保留未选”开关；悬停时会根据当前勾选状态提示下一步操作或未选提示词的保存结果。联想列表没有高亮候选时，Tab 可在文本模式与标签模式之间切换；存在高亮候选时 Tab 仍优先确认候选，Shift+Tab 保留焦点导航。右侧依次显示“字号”文字、12–30px 调整滑杆、实时像素值，以及与联想浮层一致的红色关闭按钮。字号会保存在本地。编辑器最小宽度为 600px，可拖动标题栏非交互区域移动，也可从右下角调整尺寸；再次打开时会恢复保存的窗口位置与尺寸，并自动限制在当前视口范围内。
