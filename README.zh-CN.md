...
卡片 Prompt 输入框、“+”添加框和自由模式共用双源提示词联想：**Danbooru** 来源使用 Prompt-Weaver 自管本地 CSV；**Prompt Assistant** 来源读取本机已安装 [ComfyUI-Prompt-Assistant](https://github.com/yawiii/comfyui_prompt_assistant) 暴露的全部 CSV。两个来源默认同时开启，并在 ComfyUI 设置中放入同一个圆角来源分组；可通过拖拽手柄调整优先级，也可独立关闭任一来源，位于上方的来源在匹配质量相同时优先。候选依次按完整相等、开头匹配、连续子串和字符跳跃匹配排序，之后才应用来源优先级。字符跳跃会忽略空格、下划线和连字符，并优先首次命中位置更靠前、跳过字符更少、候选更短的结果；Danbooru 同源结果再按使用量降序。最终写入内容会跨来源去重。
