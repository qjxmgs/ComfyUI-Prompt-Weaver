...
点击 Prompt 输入框右侧的编辑按钮会把当前内容按顶层中英文逗号和换行拆成标签；括号、方括号、花括号、引号及转义内容内部不会被拆分。编辑器会忽略大小写自动合并重复标签，保留首次出现项的原始文本和顺序。标签列表末尾的“+”可展开输入框，按 Enter、输入框失焦或直接点击“确认”时，会使用相同规则自动拆分并添加多个提示词；重复项不会再次添加，未选中的重复标签会被重新启用。点击或划过标签可切换选择状态，未选中标签会置灰。“保留未选”按卡片保存且默认开启：点击确认后未选标签仍留在提示词组中，但不会进入节点输出；标签右上角的红色 `×` 可将其从当前草稿移除。自由模式的 textarea 只编辑实际输出原文，并在下方以弱化标签显示保留的未选项。关闭“保留未选”后，未选项只在点击确认时丢弃；关闭按钮、弹窗级 `Esc` 或点击遮罩仍会撤销全部草稿修改。底部“复制”按钮只复制当前已选输出，且不关闭窗口或保存修改。
