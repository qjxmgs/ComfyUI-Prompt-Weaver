...
工具栏中的“选择存档…”下拉框可快速加载已保存的完整网格状态；“存档管理”用于新建、覆盖、重命名、加载、删除、导入和导出存档。一个存档包含列数、卡片顺序、开关、标题和 Prompt，但不包含节点尺寸、画布位置或连线。

- 当前状态与某个存档一致时，下拉框会自动选中该存档。比较时忽略卡片内部 ID，只比较列数及有序的开关、标题和 Prompt。
- 已加载的存档发生修改后保留当前名称并显示为 `* 存档名`；所有下拉选项都预留相同的标记宽度，状态变化时文字不会跳动。节点从未关联存档或当前存档已删除时才显示 `* 未保存`。此时切换存档会先显示自定义确认层。
- “保存当前”使用输入的名称新建存档；名称去除外围空白后必须为 1–80 个字符且不区分大小写唯一。同名保存会先询问是否覆盖。
- 覆盖和删除必须二次确认。删除当前存档不会改变节点内容，但节点会变为未保存状态。
- 同一页面中的所有网格节点会同步存档列表；其他浏览器标签页通过 `BroadcastChannel` 收到通知，下拉框获得焦点时也会重新获取列表。

存档写入 ComfyUI 当前用户目录下的 `ComfyUI-Prompt-Weaver/prompt-grid-archives.json`，因此可跨 Workflow 和浏览器使用，并按 ComfyUI 多用户隔离。后端通过临时文件和原子替换写入；文件损坏时会返回错误，不会静默覆盖。最多保存 100 个存档，每个存档最多 500 张卡片，并设有快照、导入文件和总文件大小限制。

单个或全部存档可导出为统一 JSON 包。导入前会显示存档数和卡片数，并可选择同名冲突策略：默认“跳过”、覆盖本地存档或自动重命名。服务端会先完整校验整批记录，任何非法记录都会取消整批导入。

存档列表不写入节点的执行 `config`，不会增加 Queue Prompt、Workflow 节点状态或图片元数据体积。新增、修改或删除存档后需要保持 ComfyUI 服务运行；安装此版本后因为新增了 Python 路由，必须重启 ComfyUI。
