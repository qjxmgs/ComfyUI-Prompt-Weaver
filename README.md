...
The backend also supports a third SQLite translation-supplement layer. At the user's direction it tracks the fixed upstream [`ffdkj/ffdkj-Danbooru_Tag-Chinese-English-Translation-Table`](https://github.com/ffdkj/ffdkj-Danbooru_Tag-Chinese-English-Translation-Table) repository through the GitHub Contents API and streams `tag.sqlite` to the local ComfyUI user directory only during a manual update. The locally bundled supplement policy overrides any stale remote Prompt Weaver manifest, and switching from a previous mirror forces a fresh download. Subsequent checks skip the download when the upstream blob is unchanged. A download must pass size, SQLite integrity, schema, row-count, and field validation before atomic replacement; failure retains the last known-good local file. At load time it queries only base-dictionary tags still missing from the primary overlay, so primary translations are never overwritten and out-of-dictionary tags, categories, and counts are never imported. The upstream repository currently declares no data license, so the manifest records `license_status: user-directed` rather than claiming MIT or redistribution rights; the manager identifies the file as installed for local use. `/prompt-weaver/tag-autocomplete/status` is a local-only read and reports primary and merged base-dictionary translation counts, coverage, plus the supplement's license status, enabled/available state, filled count, blob SHA, update time, and separate error.
