...
The backend also supports a third SQLite translation-supplement layer. It tracks the fixed [`qjxmgs/ffdkj-Danbooru_Tag-Chinese-English-Translation-Table`](https://github.com/qjxmgs/ffdkj-Danbooru_Tag-Chinese-English-Translation-Table) mirror through the GitHub Contents API and streams `tag.sqlite` only during a manual update and only when its blob changes. A download must pass size, SQLite integrity, schema, row-count, and field validation before atomic installation. At load time it queries only base-dictionary tags still missing from the primary overlay; it never overwrites primary translations or imports out-of-dictionary tags, categories, or counts. Because that repository currently declares no data license, the source remains disabled with `license_status: pending` in `data/tag_sources.json`; the manager only shows an awaiting-license explanation and provides no bypass. The remote manifest can enable it only after an authorized license is declared and the status changes to `cleared`. `/prompt-weaver/tag-autocomplete/status` is a local-only read and reports primary and merged base-dictionary translation counts, coverage, plus the supplement's license, enabled/available state, filled count, blob SHA, update time, and separate error.
